=============================================
//Coloquei novos loops sobre as partículas no estado intermediário -1
//lhe_to_dat
=========
// -*- C++ -*-
...
int main(int argc, char** argv) {
  ...
  while (reader->readEvent()) {
    ...
    //=================================================
    //Coloquei um loop aqui para buscar os valores das partículas intermediárias
    //o valor dois foi colocado para representar esta partícula e foi colocada n    //o mesmo vértice do evento colocar os fótons.aqui está a solução.
    //=============================================================
 for (int i = 0; i < reader->hepeup.NUP; ++i) {
      if (reader->hepeup.ISTUP[i] != 2) {
        //cout << reader->hepeup.ISTUP[i] << ", " << reader->hepeup.IDUP[i] << endl;
        continue;
      }
      ...
      FourVector p(reader->hepeup.PUP[i][0], reader->hepeup.PUP[i][1],
                   reader->hepeup.PUP[i][2], reader->hepeup.PUP[i][3]);
      GenParticle* gp = new GenParticle(p, reader->hepeup.IDUP[i], 2);
      gp->set_generated_mass(reader->hepeup.PUP[i][4]);
      v->add_particle_out(gp);
 }
